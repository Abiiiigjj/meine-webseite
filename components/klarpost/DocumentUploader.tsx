...
import { Camera, Upload, FileImage, AlertCircle, X, Sparkles } from 'lucide-react';
...
export default function DocumentUploader({
    ...
    if (selectedFile && !error) {
        return (
            ...
            >
                ...
                <button onClick={clearFile} className="p-2 rounded-lg hover:bg-white/10 transition-colors" aria-label={t.removeFile}>
    ...
    return (
        <div className="space-y-4">
            <input ref={fileInputRef} type="file" accept="image/jpeg,image/png,application/pdf" onChange={handleInputChange} className="hidden" />
                ...
                {isMobile ? (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-3">
                        <motion.button
                            ...
                            whileTap={{ scale: 0.98 }}
                            className="relative w-full overflow-hidden flex items-center justify-center gap-3 py-6 px-6 bg-gradient-to-r from-purple-600/80 to-pink-600/80 backdrop-blur-xl rounded-2xl text-white font-semibold text-lg border border-white/20 shadow-lg shadow-purple-500/25 transition-all duration-300"
                        >
                        ...
                        <button onClick={() => fileInputRef.current?.click()} className="w-full py-3 px-4 text-center text-sm text-white/60 hover:text-white/80 transition-colors">
                            {t.desktopButton}
                        </button>
                    </motion.div>
                ) : (
                    <motion.div
                        ...
                        className={`relative cursor-pointer rounded-2xl border-2 border-dashed transition-all duration-300 py-12 px-8 backdrop-blur-sm ${isDragging ? 'border-purple-400 bg-purple-500/10 scale-[1.02]' : 'border-white/20 hover:border-purple-400/50 hover:bg-white/5'}`}
                    >
                        <div className="absolute inset-0 rounded-2xl bg-gradient-to-b from-white/5 to-transparent pointer-events-none" />
                        <div className="relative flex flex-col items-center text-center">
                            <motion.div className={`p-4 rounded-2xl mb-4 transition-colors duration-300 ${isDragging ? 'bg-purple-500/20' : 'bg-white/5'}`} animate={isDragging ? { scale: [1, 1.1, 1] } : {}} transition={{ duration: 0.5, repeat: Infinity }}>
                                <Upload className={`w-10 h-10 transition-colors duration-300 ${isDragging ? 'text-purple-400' : 'text-white/40'}`} />
                            </motion.div>
                            <p className="text-lg font-medium text-white/90 mb-1">{t.desktopButton}</p>
                            <p className="text-sm text-white/50 mb-3">{t.dropHint}</p>
            ...
            <AnimatePresence>
                {error && (
                    <motion.div initial={{ opacity: 0, y: -10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -10, scale: 0.95 }} className="flex items-center gap-3 p-4 rounded-xl bg-red-500/10 border border-red-500/30">
                        <AlertCircle className="w-5 h-5 text-red-400 flex-shrink-0" />
                        <p className="text-red-300 text-sm">{error}</p>
                        <button onClick={clearFile} className="ml-auto p-1 hover:bg-white/10 rounded transition-colors">
